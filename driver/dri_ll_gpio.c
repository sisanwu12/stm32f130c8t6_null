 ...
 * @note 本实现遵循 STM32F1 的 CRL/CRH 配置模型，用户层采用输入模式 / 输出模式 /
 * 输出速度三枚举组合配置。 在 Cortex-M 目标上，GPIO
...
dri_ll_gpio_is_valid_input_mode(dri_ll_gpio_input_mode_t input_mode) // 判断 GPIO 输入模式是否合法
{
    ...
    return (uint8_t)((input_mode_value >=
                      (int32_t)DRI_LL_GPIO_INPUT_MODE_NONE) && // 输入模式下界限制为 NONE
                     (input_mode_value <=
                      (int32_t)DRI_LL_GPIO_INPUT_MODE_PULL_DOWN)); // 输入模式上界限制为下拉输入
}

static uint8_t dri_ll_gpio_is_valid_output_mode(
    dri_ll_gpio_output_mode_t output_mode) // 判断 GPIO 输出模式是否合法
{
    int32_t output_mode_value =
        (int32_t)output_mode; // 将输出模式枚举转换成有符号整数便于做边界检查
                             ...
                             uint32_t* cfg_bits) // 将用户层组合式配置编码成 STM32F1 4bit 配置字段
{
    ...
    input_active =
        (uint8_t)(config->input_mode != DRI_LL_GPIO_INPUT_MODE_NONE); // 判断是否启用了输入模式
    output_active =
        (uint8_t)(config->output_mode != DRI_LL_GPIO_OUTPUT_MODE_NONE); // 判断是否启用了输出模式
...
void dri_ll_gpio_init(const dri_ll_gpio_init_t* config) // 初始化一个 GPIO 引脚
{
    ...
    if (config->input_mode ==
        DRI_LL_GPIO_INPUT_MODE_PULL_UP) // 上拉输入需要先通过 ODR 对应位选择上拉
    {
        dri_ll_gpio_write_mask_raw(config->port, pin_mask, 0U); // 通过 ODR 对应位选择上拉
    }
    else if (config->input_mode ==
             DRI_LL_GPIO_INPUT_MODE_PULL_DOWN) // 下拉输入需要先通过 ODR 对应位选择下拉
    {
