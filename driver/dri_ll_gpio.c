 ...
 * @note 本实现遵循 STM32F1 的 CRL/CRH 配置模型，用户层采用单个模式枚举加输出速度配置。 在 Cortex-M 目标上，GPIO
 * 翻转操作会在短临界区内完成，以降低与中断并发访问时的状态竞争风险。
 * @version 0.1
 * @date 2026-03-23
 *
...
static uint8_t dri_ll_gpio_is_valid_mode(dri_ll_gpio_mode_t mode) // 判断 GPIO 模式是否合法
{
    int32_t mode_value = (int32_t)mode; // 将模式枚举转换成有符号整数便于做边界检查

    return (uint8_t)((mode_value >= (int32_t)GPIO_MODE_ANALOG) && // 模式下界限制为模拟输入
                     (mode_value <= (int32_t)GPIO_MODE_AF_OD));   // 模式上界限制为复用开漏输出
}
...
static uint8_t dri_ll_gpio_is_valid_speed(dri_ll_gpio_speed_t speed) // 判断 GPIO 输出速度是否合法
{
    ...
    return (uint8_t)((speed_value >= (int32_t)GPIO_SPEED_NONE) && // 速度下界限制为 NONE
                     (speed_value <= (int32_t)GPIO_SPEED_50MHZ)); // 速度上界限制为 50MHz
}

static uint8_t dri_ll_gpio_is_valid_level(dri_ll_gpio_level_t level) // 判断 GPIO 输出电平是否合法
{
    int32_t level_value = (int32_t)level; // 将电平枚举转换成有符号整数便于做边界检查

    return (uint8_t)((level_value >= (int32_t)GPIO_LEVEL_LOW) &&  // 电平下界限制为低电平
                     (level_value <= (int32_t)GPIO_LEVEL_HIGH)); // 电平上界限制为高电平
}
...
static uint8_t dri_ll_gpio_mode_is_output_like(dri_ll_gpio_mode_t mode) // 判断模式是否属于输出类
{
    switch (mode)
    {
...
dri_ll_gpio_encode_user_mode(const dri_ll_gpio_init_t* config,
                             uint32_t* cfg_bits) // 将用户层模式枚举编码成 STM32F1 4bit 配置字段
{
